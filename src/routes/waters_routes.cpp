...
void registerWaterRoutes(httplib::Server& svr, HealthBackend& backend) {
  ...
  svr.Patch(R"(/waters/(\d+))", [&backend](const httplib::Request& req, httplib::Response& res) {
    ...
    std::size_t index = 0;
    try { index = static_cast<std::size_t>(std::stoul(idStr)); } catch (...) {
      json err; err["errorMessage"] = "Invalid water id"; res.status = 400; res.set_content(err.dump(), "application/json"); return;
    }

    try {
      json j = json::parse(req.body);
      auto records = backend.getAllWater(token);
      if (index >= records.size()) { json err; err["errorMessage"] = "Record not found"; res.status = 404; res.set_content(err.dump(), "application/json"); return; }
      ...
      if (j.contains("amountMl")) newAmount = j["amountMl"].get<double>();

      bool ok = backend.updateWater(token, index, newDatetime, newAmount);
      if (!ok) { json err; err["errorMessage"] = "Failed to update water record"; res.status = 400; res.set_content(err.dump(), "application/json"); return; }

      json out; out["id"] = idStr; out["datetime"] = newDatetime; out["amountMl"] = newAmount;
  ...
  svr.Delete(R"(/waters/(\d+))", [&backend](const httplib::Request& req, httplib::Response& res) {
    ...
    if (!ok) { json err; err["errorMessage"] = "Record not found"; res.status = 404; res.set_content(err.dump(), "application/json"); return; }
