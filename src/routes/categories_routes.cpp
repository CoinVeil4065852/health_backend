    ...
    svr.Patch(R"(/category/([^/]+)/([^/]+))",
              [&backend](const httplib::Request &req, httplib::Response &res) {
                  ...
                  try {
                      ...
                      bool ok = backend.updateOtherRecord(token, categoryId, index, newDatetime,
                                                          value, newNote);
                      if (!ok) {
                          json err;
